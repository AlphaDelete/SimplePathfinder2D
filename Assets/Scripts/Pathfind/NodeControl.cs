...
public class NodeControl : MonoBehaviour {
	...
	public List<Vector3> Path(Vector3 startPos, Vector3 targetPos)
	{
		...
		if (!PhysicsEx.Raycast2D(startPos, targetPos - startPos, exitDistance))
		{
		...
		foreach(Point point in points) //Could be optimized to not go through each connection twice
		{
			foreach (Point point2 in points)
			{
				...
				if (!PhysicsEx.Raycast2D(point.GetPos(), point2.GetPos() - point.GetPos(), distance))
				{
			...
			if (!PhysicsEx.Raycast2D(targetPos, point.GetPos() - targetPos, distance2))
			{
		...
		foreach (Point point in points)
		{
			...
			if (!PhysicsEx.Raycast2D(startPos, point.GetPos() - startPos, distance))
			{
